...
string mult(string x, string y) {
    ...
    for (j = y.length() - 1; j >= 0; j--) {
        dy = y[j] - ZERO;
        // multiple the digit y[j] to x and store results into string <prod>
        // add zeros to the end of the string <prod>
        // if carry > 0 then add(answer, char(carry + '0') + prod)
        // otherwise add(answer, prod)
        carry = 0;
        for (i = x.length() - 1; i >= 0; i--){
